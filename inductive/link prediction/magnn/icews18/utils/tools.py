...
def parse_minibatch_LastFM_ep(adjlists_ua, edge_metapath_indices_list_ua, user_artist_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    idx_batch_mapped_lists_unmapped = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_ua, edge_metapath_indices_list_ua)):
        for adjlist, indices, use_mask in zip(adjlists, edge_metapath_indices_list, use_masks[mode]):
            if use_mask:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
            ...
            result_indices_lists[mode].append(result_indices)
            idx_batch_mapped_lists[mode].append(np.array([mapping[row[mode]] for row in user_artist_batch]))
            idx_batch_mapped_lists_unmapped[mode].append(np.array([row[mode] for row in user_artist_batch]))

    return g_lists, result_indices_lists, idx_batch_mapped_lists, idx_batch_mapped_lists_unmapped
...
def hits_at_k(rankings: list, k: int = 3) -> float:
    """
    ...
    """
    hits = sum(1 for rank in rankings if 0 < rank <= k)  # Count ranks ≤ k (ignoring 0s)
