...
class MAGNN_nc_layer(nn.Module):
    ...
    def forward(self, inputs):
        g_lists, features, type_mask, edge_metapath_indices_lists,target_node_indices = inputs
        ...
        for i, (g_list, edge_metapath_indices_list, ctr_ntype_layer) in enumerate(zip(g_lists, edge_metapath_indices_lists, self.ctr_ntype_layers)):
            if ctr_ntype_layer.use_minibatch:
                h[np.where(type_mask == i)[0]] = ctr_ntype_layer((g_list, features, type_mask, edge_metapath_indices_list,target_node_indices))
            else:
...
class MAGNN_nc(nn.Module):
    ...
    def forward(self, inputs, target_node_indices):
        ...
        for l in range(self.num_layers - 1):
            h, _ = self.layers[l]((g_lists, h, type_mask, edge_metapath_indices_lists,target_node_indices))
            h = F.elu(h)
        # output projection layer
        logits, h = self.layers[-1]((g_lists, h, type_mask, edge_metapath_indices_lists,target_node_indices))
